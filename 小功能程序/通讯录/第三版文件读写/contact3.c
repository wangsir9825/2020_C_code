...
// 把文件中的联系人信息加载到通讯录中
	void LoadContact(Contact** ppc)
	{
		...
		//读文件
		PeoInfo tmp = { 0 };
		while (fread(&tmp, sizeof(PeoInfo), 1, pf))//一次读一个联系人信息
		{
			ChecCapcity(ppc);
			(*ppc)->data[(*ppc)-> sz] = tmp;
			(*ppc)->sz++;
...
//保存通讯录信息到文件8
	...
	{
		FILE* pf = fopen("contact.dat","wb"); //以二进制写的形式打开文件
		if (pf == NULL)
		{
		...
		for (i = 0; i < pc->sz; i++)
		{
			fwrite(pc->data + i, sizeof(PeoInfo), 1, pf);//一次往文件中写一个人的信息
		}
		//关闭文件
		fclose(pf);
		pf = NULL;
